...
public class Excel_Utility {
	...
	{
		 FileInputStream fis2 = new FileInputStream("./src\\test\\resources\\TestData\\Book2.xlsx");
	...
	{
		FileInputStream fis = new FileInputStream("./src\\test\\resources\\TestData\\Book2.xlsx");
		Workbook book = WorkbookFactory.create(fis);
		Sheet sheet = book.getSheet(sheetName);
		Row row = sheet.createRow(rowNum);
		Cell cell = row.createCell(cellNum);
		cell.setCellValue(data);
		FileOutputStream fos = new FileOutputStream("./src\\test\\resources\\TestData\\Book2.xlsx");
		book.write(fos);
		book.close();
	}
}
